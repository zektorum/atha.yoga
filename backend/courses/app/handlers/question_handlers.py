...
from courses.app.http.resources.question_resources import CourseQuestionResource, CourseAnswerResource
from courses.app.repositories.question_repository import CourseQuestionRepository
from courses.app.http.requests.course_requests import CourseQuestionCreateRequest, CourseAnswerCreateRequest
from courses.app.services.question_service import CourseQuestionCreate, CourseAnswerCreate, CourseQuestionRemove, \
    CourseAnswerRemove
...
class CourseQuestionCreateHandler(GenericHandler):
    ...
    def post(self, request: Request, pk: int, *args: Any, **kwargs: Any):
        data = self.serializer_class(data=request.data)
        data.is_valid(raise_exception=True)

        question = CourseQuestionCreate(course_id=pk, user=request.user, title=data.validated_data["title"], text=data.validated_data["text"]).create()
...
class CourseAnswerCreateHandler(GenericHandler):
    ...
    def post(self, request: Request, pk: int, *args: Any, **kwargs: Any):
        ...
        answer = CourseAnswerCreate(question_id=pk, user=request.user, text=data.validated_data["text"]).create()
        return Response({"answer": CourseAnswerResource(answer).data})


class CourseQuestionListHandler(Handler):
    def get(self, request: Request, pk: int, *args: Any, **kwargs: Any):
        questions = CourseQuestionRepository().find_questions_by_course_id(course_id=pk)
        return Response(Pagination(resource=CourseQuestionResource, data=questions, request=request).paginate())


class CourseAnswerListHandler(Handler):
    def get(self, request: Request, pk: int, *args: Any, **kwargs: Any):
        answers = CourseQuestionRepository().find_answers_by_question_id(question_id=pk)
        return Response(Pagination(resource=CourseAnswerResource, data=answers, request=request).paginate())


@permission_classes([IsAuthenticated])
class CourseQuestionRemoveHandler(Handler):
    def delete(self, request: Request, pk: int, *args: Any, **kwargs: Any):
