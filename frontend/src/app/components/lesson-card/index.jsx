...
const LessonCard = ({ title, description, price, level }) => (
  <Box
    width="100%"
      ...
      <Grid item xs container direction="column" sx={{ p: '24px' }}>
        ...
        <Grid item sx={{ flex: '1 0 auto' }}>
          <Typography
            ...
            sx={{ mb: '24px' }}
          >
            {level === 'STARTING' ? 'Начинающий ' : ''}
            {level === 'STARTING' && level === 'MEDIUM' && ('•')}
            {level === 'MEDIUM' ? 'Средний' : ''}
            {level === 'ADVANCED' && level === 'MEDIUM' && ('•')}
        ...
        <Grid item xs container sx={{ mb: '24px' }}>
          ...
          <StarBorderIcon sx={{ color: '#FF9800', width: '16px', height: '16px', transform: 'translateY(2px)' }} />
          <Typography variant="body1" sx={{ fontWeight: '500' }}>
            4.8
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mr: '17px' }}>
            (505 оценок)
          </Typography>
          <ModeCommentOutlinedIcon color="text.secondary" sx={{ width: '14px', height: '14px', transform: 'translateY(4px)' }} />
          <Typography variant="body1" sx={{ fontWeight: '500' }}>
            505
          </Typography>
        </Grid>
